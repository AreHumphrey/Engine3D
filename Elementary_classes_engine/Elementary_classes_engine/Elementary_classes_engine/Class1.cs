...
namespace Elementary_classes_engine
{
    public class Program
    {
            ...
            public void Draw()
            {
                ...
                for (int y = 0; y < screenHeight; y++)
                {
                    for (int x = 0; x < screenWidth ; x++)
                    {
                        ...
                        camera.lookAt = vSpace.dir1 * ((double)x / screenWidth - 0.5) + vSpace.dir2 * ((double)y / screenHeight - 0.5) + vSpace.dir3;
                        ...
                        foreach (var obj in nearestObjects)
                        {
                            if (obj != null)
                            {
                                Ray ray = new Ray(camera.position, camera.lookAt);
                                Point intersection = obj.Intersect(ray);
        ...
        public class Spectator
        {
            ...
            public Camera Rotate(Angle angle)
            {
                Vector newRotation = new Vector(camera.rotation.x + angle.xz, camera.rotation.y + angle.yz, camera.rotation.z + angle.xy);
                camera.rotation = newRotation;
                camera.lookAt = new Vector(Math.Sin(newRotation.x * Math.PI / 180), Math.Tan(newRotation.y * Math.PI / 180), Math.Cos(newRotation.x * Math.PI / 180) * Math.Cos(newRotation.y * Math.PI / 180));
                return camera;
            }
